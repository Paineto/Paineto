<?php 
	...
	if(isset($_POST['setaction'])){
		if ($_POST['setaction']=="modify"){
			header("Location: ../my/modifygroup.php?id=".$id);
			exit;
		}
		else if ($_POST['setaction']=="delete"){
			$group->delete_team($id);
		}
	  }
?>
	<div id="container">
		...
		<form id="main" name="form" class="wufoo " autocomplete="on" method="POST">
			...
			<p>Click <a href="addgroup.php">here</a> to go ADD new group.</p>
			<center><h1><font color='maroon'>Groups</font></h1>
				<table cellspacing='2' cellpadding='0' width='800'>
					<tr>
						<th bgcolor=#5888C2><font face='Arial' color='white'>Name</th>
						<th bgcolor=#5888C2><font face='Arial' color='white'>Teamlead</th>
						<th bgcolor=#5888C2><font face='Arial' color='white'>Status</th>
						<th bgcolor=#5888C2><font face='Arial' color='white'>Actions</th>
<?
	$color="#E2FEF0";
	$result = $group->displayt();
	while($row = mysql_fetch_object($result)){
		if($color=="#DDFCDA"){
			echo "<tr bgcolor=$color>";
			echo   '    <td height="2px">'.$row->teamname.'</td>
					    <td height="2px">'.$row->teamlead.'</td>
					    <td height="2px">'.$row->status.'</td>
					    <td height="2px"><input type="image" alt="delete" src="images/delete.png" name="image" onclick="checkDelete('.$row->id.');"  width="20" height="20">Delete<input type="image" alt="modify" src="images/modify.png" name="image" onclick="checkmodify('.$row->id.');"  width="20" height="20">Modify</td>';
			$color="#E2FEF0";
		}
		else{
			echo "<tr bgcolor=$color>";
