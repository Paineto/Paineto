...
if (array_key_exists('sub', $_POST)) {
		...
		header('Refresh: 0; URL=sales.php?note=1');
		}
		else {
			$totalaccount=$cost*$qty;

			$query =("INSERT INTO invoice (item,Total,whouse , transdate, typeid,pid,qty)
			VALUES ('$Item',  '$totalaccount','$whouse', '$date',1, '$pid','$qty')")or die(mysql_error());
			mysql_query($query);

			$query = ("UPDATE stock set qty = qty - $qty where name='$Item'") or die(mysql_error());
			mysql_query($query);

			$account1=mysql_query("select * from account where accid = $pid")or die(mysql_error());
		
			if (mysql_num_rows($account1) > 0){
			$query = ("UPDATE  account set account = account + $totalaccount where accid=$pid") or die(mysql_error());
			}
			else{
			$query = ("insert into account values('','$pid',1,'$totalaccount')") or die(mysql_error());
			}
			mysql_query($query);
			header('Refresh: 0; URL=sales.php?note=2');	
		}
}
...
<form id="sales" name="sales"  autocomplete="on" method="POST" >
...
<?}?>
<div >
	<h4><a href="reportinvoice.php?type=1">Go To Report</a></h4>
</div>
<ul>
	...
	<li>
		...
		<div>
			<input id="a" type="text" onchange="searchitem()" name="a" size="6" class="field text medium"  >
				<select name="selectItem" onchange="setitem()" value="" size=1 >
<? 
...
<li>
	...
	<div>
		<input type="text" name="b" onchange="searchpeople()" size="6"class="field text medium" >
		<select name="Custumer"  size=1 >
<?
 if(isset($_POST['setaction'])){
    $member = $_POST["Custumer"];
    if ($_POST['setaction']=="people"){
	    $result = $group->display(1,$_POST['b']);
		while($row = mysql_fetch_object($result))
		{
...
<li>
	...
	<li class="buttons">
				...
				<input id="savesales" class="btTxt submit"  type="submit"  name="sub" value="Submit" />
